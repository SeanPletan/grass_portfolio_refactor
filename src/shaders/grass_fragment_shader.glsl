...
vec3 lambertLight(vec3 normal, vec3 viewDir, vec3 lightDir, vec3 lightColour) {
    float wrap = 0.5;  //the more you increase this value, the more it wraps the light around the edges
...
vec3 phongSpecular(vec3 normal, vec3 lightDir, vec3 viewDir) {
    ...
    phongValue = pow(phongValue, 256.0); //arbitrary
...
void main() {
    ...
    //Fake AO
    float ao = remap(grassY + 0.2, 0.0, 1.0, 0.125, 1.0);
